class Statistic < ActiveRecord::Base
  ...
  validate :sum_hourly_daily_distribution_equal_100_percent


  def sum_hourly_daily_distribution_equal_100_percent
    sum = 0
    24.times{|hour| sum += eval("hourly_daily_distribution#{hour}")}
    if 100 != sum


    errors.add(:hourly_daily_distribution1, "total hourly distribution is not 100%(#{sum})")
    end
  end
